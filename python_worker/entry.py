...
This module is launched as a subprocess by the TS WorkerProcess (src/workers/).
It hosts a WorkerHandler implementation that provides tool-specific logic
(Python execution, R execution, etc.).

Launch pattern:
    python -m python_worker.entry <handler_class_path> [--key value ...]

Examples:
    python -m python_worker.entry python_worker.handlers.PythonHandler --work_dir /path/to/session
    python -m python_worker.entry python_worker.handlers.RHandler --work_dir /path/to/session
...
def _import_class(dotted_path: str):
    """Import a class from a 'module.ClassName' dotted path."""
...
def main():
    ...
    # Redirect sys.stdout/stderr so stray prints from imports or handler setup
    # don't corrupt the protocol channel.
    import io
