...
class RHandler(WorkerHandler):
    ...
    def _get_state(self) -> List[Dict[str, Any]]:
        ...
        def _r_check(func_name: str, obj) -> bool:
            try:
                return bool(self._robjects.r[func_name](obj)[0])
            except Exception:
                return False

        def _r_class_str(obj) -> str:
            try:
                return str(self._robjects.r['class'](obj)[0])
            except Exception:
                return "unknown"

        def _r_capture_preview(obj, max_chars: int = 2000) -> str:
            try:
        ...
        with self._localconverter(
            ...
            for name in list(self.env.keys()):
                if name.startswith('.') or name in _EXCLUDED_VARS:
                    continue

                obj = self.env[name]
                r_class = _r_class_str(obj)
                value_info = ""
                preview_content = ""
                is_error = False

                try:
                    if (_r_check('is.ggplot', obj)
                    ...
                    elif _r_check('is.data.frame', obj):
                        r_class = "data.frame"
                        ...
                        try:
                            ...
                            if hasattr(pd_df, 'to_html'):
                                ...
                                preview_content = _r_capture_preview(obj)
                        except Exception:
                            preview_content = _r_capture_preview(obj)

                    elif _r_check('is.matrix', obj):
                        r_class = "matrix"
                        try:
                    ...
                    elif _r_check('is.function', obj):
                        r_class = "function"
                        preview_content = _r_capture_preview(obj)

                    elif _r_check('is.factor', obj):
                        r_class = "factor"
                        try:
                    ...
                    elif _r_check('is.atomic', obj):
                        if _r_check('is.integer', obj):
                            r_class = "integer"
                        elif _r_check('is.numeric', obj):
                            r_class = "numeric"
                        elif _r_check('is.character', obj):
                            r_class = "character"
                        elif _r_check('is.logical', obj):
                            r_class = "logical"
                        try:
                            length = int(self._robjects.r['length'](obj)[0])
                            value_info = f"({length})"
